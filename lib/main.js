...
function PageWorker(id)
{
    ...
    var dsScript =
        ...
        "current_dgs_debug_level = " + SearchPrefs.current_main_debug_level + ";\n" +
    ...
    this.worker = require("sdk/page-worker").Page({
...
var SearchManager = {
    ...
    updateSearchList: function(ts, te, n, query)
    {
        ...
        var remainQ = SearchPrefs.queueSize - this.searchVector.length; 
        if(remainQ <= 0)
        {
            main_dblog(DEBUG_LEVEL.INFO, "updateSearchList queue is full");
            return;
        }
        else
        {
            ...
            {
                n = remainQ;
                main_dblog(DEBUG_LEVEL.INFO, "updateSearchList queue is full, #new changed to " + n);
            }
        }
        ...
        // adjust the time so neighboring diluted searches are at least minGap ms away from each other
        // if minGap is too small, google will send out warnings
        var minGap = SearchPrefs.minSearchGap * 1000;
        if(this.searchVector.length > 1)
        {
            for (var i = 0; i < this.searchVector.length - 1; i++) 
            {
                if((this.searchVector[i+1].time - this.searchVector[i].time) < minGap)
                {
                    // to keep some randomness, add minGap to i+1 instead of i 
                    this.searchVector[i+1].time = this.searchVector[i+1].time + minGap;
                }
            }
        }
        
        main_dblog(DEBUG_LEVEL.DEBUG, "new scheduledList" + JSON.stringify(this.searchVector));
        // log
...
var SearchPrefs = {
    ...
    init: function()
    {
        ...
        // search min time delay control
        require("sdk/simple-prefs").on("minSearchGap", SearchPrefs.onMinSearchGapChange);
        this.onMinSearchGapChange('minSearchGap');
        main_dblog(DEBUG_LEVEL.INFO, "SearchPrefs.minSearchGap: Initial min search gap is " + this.minSearchGap);

        // search queue size control
        require("sdk/simple-prefs").on("queueSize", SearchPrefs.onQueueSizeChange);
        this.onQueueSizeChange('queueSize');
        main_dblog(DEBUG_LEVEL.INFO, "SearchPrefs.queueSize: Initial queue size is " + this.queueSize);
        ...
        // search history size
        require("sdk/simple-prefs").on("searchHistorySize", SearchPrefs.onSearchHistorySizeChange);
        this.onSearchHistorySizeChange('searchHistorySize');
        main_dblog(DEBUG_LEVEL.INFO, "SearchPrefs.init Search history size is " + this.searchHistorySize);
    ...
    onMinSearchGapChange: function(prefName)
    {
        var minSearchGap = require("sdk/simple-prefs").prefs["minSearchGap"];
        main_dblog(DEBUG_LEVEL.WARNING, "onMinSearchGapChange " + prefName + " was changed to " + minSearchGap);
        main_dblog(DEBUG_LEVEL.WARNING, "onMinSearchGapChange old number was " + SearchPrefs.minSearchGap);
        SearchPrefs.minSearchGap = minSearchGap;
    },

    onQueueSizeChange: function(prefName)
    {
        var queueSize = require("sdk/simple-prefs").prefs["queueSize"];
        main_dblog(DEBUG_LEVEL.WARNING, "onQueueSizeChange " + prefName + " was changed to " + queueSize);
        main_dblog(DEBUG_LEVEL.WARNING, "onQueueSizeChange old number was " + SearchPrefs.queueSize);
        SearchPrefs.queueSize = queueSize;
    },
    ...
    onSearchHistorySizeChange: function(prefName)
    {
        var searchHistorySize = require("sdk/simple-prefs").prefs["searchHistorySize"];
        main_dblog(DEBUG_LEVEL.WARNING, "onSearchHistorySizeChange " + prefName + " was changed to " + searchHistorySize);
        main_dblog(DEBUG_LEVEL.WARNING, "onSearchHistorySizeChange old value was " + SearchPrefs.searchHistorySize);
        SearchPrefs.searchHistorySize = searchHistorySize;
    },
        ...
        if (topic == "http-on-modify-request")
        {
            ...
            if(channel && url)
            {
                ...
                if(url.indexOf(SearchPrefs.searchURLPrefix) == 0)
                {
                    ...
                    main_dblog(DEBUG_LEVEL.INFO, "@@@@@@@@ toolbar search:" + url + ' redirect:' + redirectURL);
                    ...
                    require("sdk/timers").setTimeout(function callback(e){
                        SearchManager.updateSearchList(new Date().getTime(),  
                                                       ...
                                                       decodeURIComponent(url.substring(url.lastIndexOf('&q=')+3).replace('+', '%20')));
                    }, 0);
                }
            }
        }
